@testset "edge cases" begin
    ...
    @test r.n == 10
    @test any(occursin("Missing", L) || occursin("omitted", L) for L in r.limitations) ||
          r.n == 10
    ...
    @test occursin("robust", lowercase(cp.selection_reason)) ||
          cp.method === :robust_median ||
          cp.method isa Symbol

    rows = [(; value = 1.0, timestamp = DateTime(2026, 1, 1))]
    stream = AssaySentinel.from_table(
        rows;
        analyte = :x,
        unit = "U",
        value = :value,
