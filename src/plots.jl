...
function svg_control_chart(timestamps::AbstractVector, values::AbstractVector;
    ...
    for (k, y, col, dash) in (
        ...
        println(
            io,
            """<line x1="48" y1="$yy" x2="$(W - 12)" y2="$yy" stroke="$col" stroke-width="$(k == 0 ? 1.6 : 0.8)"$ds/>""",
        )
    end
    pts = IOBuffer()
    ...
    println(
        io,
        """<polyline fill="none" stroke="$_NAVY" stroke-width="1.2" points="$(String(take!(pts)))"/>""",
    )
    for e in events
        t = event_time(e)
        ...
        println(
            io,
            """<line x1="$xv" y1="32" x2="$xv" y2="$(H - 20)" stroke="$_TEAL" stroke-width="1" stroke-dasharray="3 2"/>""",
        )
        println(
            io,
            """<text x="$(xv + 3)" y="42" fill="$_TEAL" font-size="9" font-family="Segoe UI, sans-serif">$(_svg_esc(event_kind(e)))</text>""",
        )
    end
    for t in changepoints
        ...
        println(
            io,
            """<line x1="$xv" y1="32" x2="$xv" y2="$(H - 20)" stroke="$_AMBER" stroke-width="1.6"/>""",
        )
    end
    println(io, "</svg>")
    String(take!(io))
...
function svg_timeline(beats::Vector{StoryBeat})
    ...
    println(
        io,
        """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 $W $H" role="img" aria-label="Analytical reconstruction timeline">""",
    )
    println(io, """<rect width="$W" height="$H" fill="$_CREAM"/>""")
    println(
        io,
        """<text x="16" y="18" fill="$_NAVY" font-family="Segoe UI, sans-serif" font-size="13">Reconstruction timeline</text>""",
    )
    for (i, b) in enumerate(beats)
        ...
        println(
            io,
            """<rect x="$x" y="36" width="$(bw - 16)" height="44" rx="6" fill="$fill"/>""",
        )
        println(
            io,
            """<text x="$(x + 8)" y="62" fill="$_CREAM" font-size="11" font-family="Segoe UI, sans-serif">$(_svg_esc(b.label))</text>""",
        )
        if i < n
            ax = x + bw - 14
...
function svg_provenance(records::Vector{ProvenanceRecord})
    ...
    println(
        io,
        """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 $W $H" role="img" aria-label="Provenance graph">""",
    )
    println(io, """<rect width="$W" height="$H" fill="$_CREAM"/>""")
    println(
        io,
        """<text x="16" y="18" fill="$_NAVY" font-family="Segoe UI, sans-serif" font-size="13">Provenance</text>""",
    )
    for (i, r) in enumerate(records)
        y = 14 + i * row
        println(
            io,
            """<rect x="40" y="$(y - 14)" width="$(W - 60)" height="28" rx="4" fill="$_NAVY"/>""",
        )
        label = "$(r.operation) → $(r.func)  [$(r.statement_kind)]  fp=$(r.input_fingerprint)"
        println(
            io,
            """<text x="50" y="$(y + 4)" fill="$_CREAM" font-size="10" font-family="Consolas, monospace">$(_svg_esc(label))</text>""",
        )
        if i < n
            println(
                io,
                """<line x1="52" y1="$(y + 14)" x2="52" y2="$(y + row - 14)" stroke="$_TEAL" stroke-width="2"/>""",
            )
        end
    end
    println(io, "</svg>")
...
function svg_group_chart(labels::AbstractVector, values::AbstractVector;
    ...
    for (k, g) in enumerate(groups)
        ...
        println(
            io,
            """<line x1="$(cx - 18)" y1="$(mapy(q1))" x2="$(cx - 18)" y2="$(mapy(q3))" stroke="$_TEAL" stroke-width="6"/>""",
        )
        println(
            io,
            """<line x1="$(cx - 26)" y1="$(mapy(med))" x2="$(cx - 10)" y2="$(mapy(med))" stroke="$_NAVY" stroke-width="2"/>""",
        )
        for (j, y) in enumerate(gv)
            jitter = ((j % 7) - 3) * 1.6
            println(
                io,
                """<circle cx="$(cx + 8 + jitter)" cy="$(mapy(y))" r="2.2" fill="$_NAVY" fill-opacity="0.55"/>""",
            )
        end
        println(
            io,
            """<text x="$cx" y="$(H - 12)" text-anchor="middle" fill="$_NAVY" font-size="11" font-family="Segoe UI, sans-serif">$(_svg_esc(g))</text>""",
        )
    end
    println(io, "</svg>")
    String(take!(io))
