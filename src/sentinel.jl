...
function update!(sentinel::Sentinel, measurement)
    ...
    if alert(r)
        ...
        cooled = sentinel.last_alert === nothing ||
...
function analyze(stream::AssayStream;
    ...
    record_step!(prov;
        ...
        parameters = Dict("method" => string(outliers.method), "policy" => string(outlier_policy)),
        input_fingerprint = fingerprint(finite),
    ...
    record_step!(prov;
        ...
        parameters = Dict("kind" => string(drift.kind), "detector" => string(drift.detector)),
        input_fingerprint = fingerprint(work),
        parent_ids = [parent.id],
    ...
    qc_pen = isempty(qc_results) ? 0.0 :
             count(r -> r.triggered && r.severity in (:warning, :critical), qc_results) / max(length(qc_results), 1)
    ...
    miss > 0 && push!(limitations, "Missing/NaN fraction $(round(miss; digits=3)) was omitted, not imputed.")
...
function analyze(values::AbstractVector;
    ...
    ts = timestamps === nothing ? [now() + Second(i) for i in 1:length(values)] : collect(timestamps)
...
function Base.show(io::IO, r::QualityReport)
    ...
    sign = r.drift.direction === :increase ? "+" : r.drift.direction === :decrease ? "−" : ""
    println(io, isempty(sign) ? string(r.drift.direction) : @sprintf("%s%.1f%%", sign, abs(magpct)))
...
function Base.summary(panel_result::NamedTuple)
    if haskey(panel_result, :reports)
        rows = [(analyte = k, status = _status_label(v.status), score = round(v.score.value; digits = 1))
